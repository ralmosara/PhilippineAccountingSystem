...
<head>
    <meta charset="UTF-8">
    <title>Cash Receipts Book — {{ $payload['period'] }}</title>
    <style>
...
<body>
    ...
    <div class="period"><strong>CASH RECEIPTS BOOK</strong> · For the period {{ $payload['period'] }}</div>

    <table>
        <thead>
            <tr>
                <th style="width: 90px">Date</th>
                <th style="width: 120px">OR No.</th>
        ...
        <tbody>
            ...
            @endforeach
            <tr class="totals">
                <td colspan="5">TOTAL ({{ $payload['row_count'] }} receipts)</td>
    ...
    <div style="margin-top: 16px; font-size: 8pt; color: #666;">
        Generated {{ now()->format('Y-m-d H:i T') }} · Voided ORs excluded; sequence preserved.
    </div>
</body>
</html>
